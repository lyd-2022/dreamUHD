...
class DreamUHD(nn.Module):
    ...
    @torch.no_grad()
    def test_tile(self, input, tile_size=1024, tile_pad=16):
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            self.adapters.insert(0, WTA_decoder(dwt_dim,dwt_dim, block_out))
...
class WTA_decoder (nn.Module):
    def __init__(self, in_channels, out_channels,enc_channel, kernel_size=5, stride=1):
        super(WTA_decoder, self).__init__()
    ...
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        for i_level in range(self.num_resolutions):
            ...
            if i_level != self.num_resolutions - 1:
                ...
                self.adapters.append(WTA_encoder(dwt_dim,dwt_dim,block_out))
...
if __name__== '__main__': 
    ...
    # srun -p ai4earth --job-name=try --gres=gpu:0 --ntasks-per-node=1 python /mnt/petrelfs/liuyidi/Code/dreamUHD/basicsr/archs/DreamUHD_arch.py