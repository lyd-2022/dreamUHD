...
def make_FE(in_channels, out_channels,temb_channels,dropout, res_type="vanilla"):
    assert res_type in ["vanilla", "FEblock","none"], f'res_type {res_type} unknown'
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        # middle
        self.mid = nn.Module()
        self.mid.block_1 = make_FE(in_channels=block_in,
                                       out_channels=block_in,
                                       temb_channels=self.temb_ch,
                                       dropout=dropout,res_type=res_type)
        self.mid.attn_1 = make_attn(block_in, attn_type=attn_type)
        self.mid.block_2 = make_FE(in_channels=block_in,
                                       out_channels=block_in,
                                       temb_channels=self.temb_ch,
                                       dropout=dropout,res_type=res_type)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for i_block in range(self.num_res_blocks+1):
                block.append(make_FE(in_channels=block_in,
...
if __name__ == '__main__':
    import yaml
    with open("/mnt/petrelfs/liuyidi/Code/dreamUHD/experiments/LL/VAE/VAE.yml") as f:
        config = yaml.load(f, Loader=yaml.FullLoader)["network_g"]
    config.pop('type')
    # config['ddconfig']['res_type'] = "Res_four2"
    # model = FEVAE(**config).cuda()
    model = FEVAE(**config)
    param = torch.load('/mnt/petrelfs/liuyidi/Code/dreamUHD/experiments/LL/VAE/VAE_LL.pth')

    model.load_state_dict(param['params'], strict=True)
...
# srun -p ai4earth --job-name=try --gres=gpu:0 --ntasks-per-node=1 python /mnt/petrelfs/liuyidi/Code/dreamUHD/basicsr/archs/FEVAE_arch.py