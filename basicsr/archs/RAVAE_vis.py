...
class RAVAE(nn.Module):
    ...
    def forward(self, input):
        #采样过程合并到encode中
        z,scale = self.encode(input,self.sample_posterior)
        # dec = self.decode(z)
        return z, scale
    ...
    def test_tile(self, input, tile_size=1024, tile_pad=16):
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                # clean_tile = clean[:, :, input_start_y_pad:input_end_y_pad, input_start_x_pad:input_end_x_pad]

                # upscale tile
                output_tile = self(input_tile)[0]
...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
                h = self.up[i_level].upsample(h)+h1
                del h1
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        scale = [hs[-1]]
        for i_level in range(self.num_resolutions):
            h1 = self.shuffler(hs[-1])
            h10 ,h11 = h1.chunk(2,dim=1)
            h1 = torch.mean(torch.stack([h10, h11]), dim=0)
            del h10,h11
            for i_block in range(self.num_res_blocks):
                ...
                hs.append(h)
            if i_level != self.num_resolutions - 1:
                hs.append(self.down[i_level].downsample(hs[-1])+h1)
                scale.append(h1)
                del h1
        # middle
        h = hs[-1]
...
if __name__ == '__main__':
    import yaml
    with open("/mnt/petrelfs/liuyidi/Code/dreamUHD/options/stage1/RAVAE.yml") as f:
