...
class SAFMN_adenc6kl8(nn.Module):
    ...
    def forward(self, input):
        x0 = self.to_feat(input)
        x = self.vae.encode(input)
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 ...
                 attn_type="restormer", **ignorekwargs):
        super().__init__()
...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.resolution = resolution
        self.in_channels = in_channels
        self.adapter_wtconv1 = nn.Sequential(WTConv2d(3, 3, kernel_size=5, stride=1, bias=True, wt_levels=2, wt_type='db1'),
                                             NAFBlock(3))
        
        self.adapter_wtconv2 = nn.Sequential(WTConv2d(3, 3, kernel_size=5, stride=1, bias=True, wt_levels=2, wt_type='db1'),
                                                NAFBlock(3))
        self.adapter_wtconv3 = nn.Sequential(WTConv2d(3, 3, kernel_size=5, stride=1, bias=True, wt_levels=2, wt_type='db1'),
                                                NAFBlock(3))
        self.adapter_wtconv4 = nn.Sequential(WTConv2d(3, 3, kernel_size=5, stride=1, bias=True, wt_levels=2, wt_type='db1'),
                                                NAFBlock(3))
        # self.adapter_wtconv5 = nn.Sequential(WTConv2d(3, 3, kernel_size=5, stride=1, bias=True, wt_levels=2, wt_type='db1'),
        #                                         NAFBlock(3))
        adapter_dim = [3+16,32+12,64+48,64+192]
        # downsampling
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                block.append(ResnetBlock(in_channels=block_in,
                                         ...
                                         temb_channels=self.temb_ch,
                                         dropout=dropout))
                adapters.append(Adapter(adapter_dim[i_level], block_out))  # Add Adapter
                block_in = block_out
                if curr_res in attn_resolutions:
                    attn.append(make_attn(block_in, attn_type=attn_type))
            down = nn.Module()
        ...
        self.mid.block_2 = ResnetBlock(in_channels=block_in,
                                       ...
                                       temb_channels=self.temb_ch,
                                       dropout=dropout)
        self.mid.adapter = Adapter(block_in, block_in)  # Add Adapter
    ...
    def forward(self, x):
        # timestep embedding
        temb = None
        adapter_in1 = self.adapter_wtconv1(x) + x
        adapter_in2 = self.adapter_wtconv2(adapter_in1) + adapter_in1
        adapter_in3 = self.adapter_wtconv3(adapter_in2) + adapter_in2
        adapter_in4 = self.adapter_wtconv4(adapter_in3) + adapter_in3
        # adapter_in5 = self.adapter_wtconv5(adapter_in4) + adapter_in4
        ...
        adapter_in = [adapter_in1, adapter_in2, adapter_in3, adapter_in4]
        # downsampling
        hs = [self.conv_in(x)]
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks):
        ...
        h = hs[-1]
        h = self.mid.block_1(h, temb)
        h = h + self.mid.adapter(h)  # Apply Adapter
        h = self.mid.attn_1(h)
        h = self.mid.block_2(h, temb)
...
if __name__== '__main__': 
    ...
    model = SAFMN_adenc6kl8(dim=48, n_blocks=8, ffn_scale=2.0, upscaling_factor=8,
                         vae_weight='/data/liuyidi/nitre_2023_dehaze/data_dehaze/uhdformer/experiments/resfa_8_16_2(2)/models/net_g_latest.pth',
                         config='/data/liuyidi/nitre_2023_dehaze/data_dehaze/uhdformer/experiments/resfa_8_16_2(2)/8_16.yml').to('cuda')
