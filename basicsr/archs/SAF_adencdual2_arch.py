...
class SAF_adencdual2(nn.Module):
    def __init__(self, dim, n_blocks=8, ffn_scale=2.0, upscaling_factor=4,vae_weight=None,config=None,sample= True):
        ...
        self.feats = nn.Sequential(*[AttBlock(16, ffn_scale) for _ in range(n_blocks)])
        self.feats2 = nn.Sequential(*[AttBlock(192, ffn_scale) for _ in range(n_blocks)])

        self.to_img = nn.Sequential(
            nn.Conv2d(192, dim, 3, 1, 1),
    ...
    def forward(self, x):
        x0 = self.to_feat(x)
        x0 = self.feats2(x0) + x0
        x0 = self.to_img(x0)


        x1 = self.vae.encode(x,self.sample)[0]
        x1 = self.feats(x1) + x1 #[1, 64, 240, 135]
        x1 = self.merge(x1)
        x1 = self.vae.decode(x1)
...
if __name__== '__main__': 
    x = torch.randn(1, 3, 2160, 1280).to('cuda')
    model = SAF_adencdual2(dim=48, n_blocks=8, ffn_scale=2.0, upscaling_factor=4,
                         vae_weight='/data/liuyidi/nitre_2023_dehaze/data_dehaze/uhdformer/experiments/KL_16_32/models/net_g_latest.pth',
                         config='/data/liuyidi/nitre_2023_dehaze/data_dehaze/uhdformer/experiments/KL_16_32/kl_16.yml').to('cuda')
