...
# from basicsr.archs.VAE_fu_pad_arch import AutoencoderKL_freup
import time
import yaml
...
class SAF_adencdual3kl8(nn.Module):
    def __init__(self, dim, n_blocks=8, ffn_scale=2.0, upscaling_factor=4,vae_weight=None,config=None,sample= True):
        ...
        self.conv0 = nn.Conv2d(3,8,7,1,3)
        self.high = nn.Sequential(*[
            NAFBlock(8) for i in range(9)])
        
        self.out = nn.Conv2d(11,3,7,1,3)
        

    def forward(self, input):

        x0 = self.to_feat(input)
        x = self.vae.encode(input,self.sample)[0]
...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.adapter_merge = nn.ModuleList([
            torch.nn.Conv2d(3+16, 16, kernel_size=1, stride=1, padding=0),
...
if __name__== '__main__': 
    ...
    model = SAF_adencdual3kl8(dim=48, n_blocks=8, ffn_scale=2.0, upscaling_factor=8,
