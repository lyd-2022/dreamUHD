...
class SAF_adencdual(nn.Module):
    def __init__(self, dim, n_blocks=8,n_blocks2 = 8, ffn_scale=2.0, upscaling_factor=4,vae_weight=None,config=None,sample= True):
        super().__init__()
        ...
        self.uhd =  nn.Sequential(*[AttBlock(3, ffn_scale) for _ in range(n_blocks2)])
    ...
    def forward(self, x):
        x_high = self.uhd(x)
        x0 = self.to_feat(x)
        x = self.vae.encode(x,self.sample)[0]
...
if __name__== '__main__': 
    ...
    model = SAF_adencdual(dim=48, n_blocks=8,n_block2=16, ffn_scale=2.0, upscaling_factor=16,
