...
from basicsr.archs.encoder_4 import Encoder, Decoder
...
class AutoencoderKL_fredw(nn.Module):
    def __init__(self,
                 ddconfig,    
                 embed_dim,
                 optim,
...
if __name__ == '__main__':
    ...
    with open("/code/UHDformer-main/options/VAE/kl_16_fredw.yml") as f:
        config = yaml.load(f, Loader=yaml.FullLoader)["network_g"]
    config.pop('type')
    model = AutoencoderKL_fredw(**config).cuda()
