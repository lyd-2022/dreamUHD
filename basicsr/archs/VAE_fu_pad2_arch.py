...
from basicsr.archs.encoder_6 import Encoder, Decoder
...
class AutoencoderKL_freup3(nn.Module):
    def __init__(self,
                 ddconfig,    
                 embed_dim,
                 optim,
...
if __name__ == '__main__':
    ...
    with open("/code/UHDformer-main/options/VAE/restormer+frepad/8_16.yml") as f:
        config = yaml.load(f, Loader=yaml.FullLoader)["network_g"]
    config.pop('type')
    config['ddconfig']['freup_type'] = 'Dysample'
    model = AutoencoderKL_freup3(**config).cuda()
