...
from basicsr.archs.encoder_5 import Encoder, Decoder
...
class AutoencoderKL_updown(nn.Module):
    def __init__(self,
                 ddconfig,    
                 embed_dim,
                 optim,
...
if __name__ == '__main__':
    import yaml
    with open("/code/UHDformer-main/options/VAE/kl_16.yml") as f:
        config = yaml.load(f, Loader=yaml.FullLoader)["network_g"]
    config.pop('type')
    model = AutoencoderKL_updown(**config).cuda()
