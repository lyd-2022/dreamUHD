...
class dwt_kl8_3(nn.Module):
    def __init__(self, dim, n_blocks=8, ffn_scale=2.0, upscaling_factor=4,vae_weight=None,param_key ='params',config=None,sample= True,dwt_dim = 16,num_heads=3):
        super().__init__()
    ...
    def forward(self, input):
        ...
        x,add,high_list,ll_list = self.vae.encode(input)
        ...
        x = self.vae.decode(x,add,high_list,ll_list) + input
...
class AutoencoderKL(nn.Module):
    ...
    def encode(self, x):
        h,ll ,high_list,ll_list = self.encoder(x)
        ll = self.adapter_mid(ll)
        moments = self.quant_conv(h)
        
        return moments,ll,high_list,ll_list

    def decode(self, z,ll,high_list,ll_list):
        z = self.post_quant_conv(z)
        dec = self.decoder(z,ll,high_list,ll_list)
        return dec
...
class Decoder(nn.Module):
    ...
    def forward(self, z,ll,high_list,ll_list):
...
class WTblock(nn.Module):
    def __init__(self, in_channels, out_channels,enc_channel, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1'):
        ...
        self.merge_conv = nn.Conv2d(in_channels*4+enc_channel, in_channels*4, kernel_size=3, padding=1)
    ...
    def forward(self, x,enc): 
        ...
        curr_x_tag = self.merge_conv(curr_x_tag)
...
class dwt_adapter (nn.Module):
    def __init__(self, in_channels, out_channels,enc_channel, kernel_size=5, stride=1):
        ...
        self.zeroconv = ZeroConv2d(in_channels*4, enc_channel, 3,padding=1, stride=stride)

    def forward(self, x,enc):
        x = self.wtblock(x,enc)
        ll = x[:,:,0,:,:]
        high = x[:,:,1:4,:,:]
        x = self.zeroconv(x.view(x.shape[0],-1,x.shape[3],x.shape[4]))
        return x,ll,high
...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 ...
                 resolution, z_channels,dwt_dim = 16 ,double_z=True, use_linear_attn=False, attn_type="vanilla",
                 **ignore_kwargs):
        super().__init__()
        if use_linear_attn: attn_type = "linear"
        self.ch = ch
    ...
    def forward(self, x):
        ...
        h = nonlinearity(h)
        h = self.conv_out(h)
        return h,ll,high_list,adapter_in
...
if __name__== '__main__': 
    ...
    model = dwt_kl8_3(dim=48, n_blocks=8, ffn_scale=2.0, upscaling_factor=8,dwt_dim=3,num_heads=3,
