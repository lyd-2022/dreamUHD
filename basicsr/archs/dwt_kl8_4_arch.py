...
from basicsr.archs.Dysample import DySample
...
def make_upsample(in_channels, with_conv,freup_type):
    if freup_type ==  None:
        return Upsample(in_channels, with_conv)
    elif freup_type == 'Dysample':
        print('freup_type is',freup_type)
        return DySample(in_channels)
        
    else:
        return fresadd(in_channels,freup_type)
...
class dwt_kl8_4(nn.Module):
    def __init__(self, dim, n_blocks=8, ffn_scale=2.0, upscaling_factor=4,vae_weight=None,config=None,sample= True,param_key ='params',dwt_dim = 16,num_heads=3):
        super().__init__()
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            up.attn = attn
            if i_level != 0:
                up.upsample = make_upsample(block_in, freup_type= freup_type,with_conv=False)
                curr_res = curr_res * 2
