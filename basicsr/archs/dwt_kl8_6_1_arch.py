...
class dwt_adapter (nn.Module):
    def __init__(self, in_channels, out_channels,enc_channel, kernel_size=5, stride=1):
        ...
        self.enc_conv = nn.Conv2d(in_channels*4, enc_channel, 3,padding=1, stride=stride)
        self.zeroconv = ZeroConv2d(enc_channel, enc_channel, 3,padding=1, stride=stride)
        self.ll_conv = nn.Conv2d(in_channels*4,in_channels,1)
        self.high_conv = nn.Conv2d(in_channels*4,in_channels*3,1)
...
if __name__== '__main__': 
    ...
    model = dwt_kl8_6_1(dim=48, n_blocks=8, ffn_scale=2.0, upscaling_factor=8,dwt_dim=3,num_heads=3,
