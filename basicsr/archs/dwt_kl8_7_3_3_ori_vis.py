...
class dwt_kl8_7_3_3_ori(nn.Module):
    ...
    def forward(self, input):
        x0,add,high_list = self.vae.encode(input)
        posterior = DiagonalGaussianDistribution(x0)
        if self.sample:
            x0 = posterior.sample()
        else:
            x = posterior.mode()
        ...
        x = self.feats(x0) + x0 #[1, 64, 240, 135]
...
if __name__== '__main__': 
    ...
    model = dwt_kl8_7_3_3_ori(dim=16, n_blocks=8, ffn_scale=2.0, upscaling_factor=8,dwt_dim=3,num_heads=3,
                         vae_weight='/data/liuyidi/nitre_2023_dehaze/data_dehaze/uhdformer/experiments/120kema_resfour10/models/net_g_latest.pth',
                         config='/data/liuyidi/nitre_2023_dehaze/data_dehaze/uhdformer/experiments/120kema_resfour10/8_16_res.yml').to('cuda')
