...
from basicsr.archs.Resblock.Res_four import Res_four,Res_four2,Res_four3,Res_four4,Res_four5,Res_four6,Res_four7,Res_four8,Res_four9,FEblock,Res_four11
from basicsr.archs.Nafnet import NAFNet
...
def make_res(in_channels, out_channels,temb_channels,dropout, res_type="vanilla"):
    assert res_type in ["vanilla", "Fourmer","MAB","Res_four","Res_four2","Res_four3","Res_four4","Res_four5","Res_four6","Res_four7","Res_four8","Res_four9","Res_four10","Res_four11","none"], f'res_type {res_type} unknown'
...
class dwt_kl8_7_3_3m_naf(nn.Module):
    def __init__(self, dim, width=16, middle_blk_num=2, enc_blk_nums=[2, 3, 3, 4], dec_blk_nums=[2, 3, 3, 4], upscaling_factor=4,vae_weight=None,config=None,sample= True,dwt_dim = 16,num_heads=3,param_key = 'params',mid_dim =24):
        super().__init__()
        out_dim = dim+16
        ...
        self.feats = NAFNet(img_channel=mid_dim, width=width, middle_blk_num=middle_blk_num, enc_blk_nums=enc_blk_nums, dec_blk_nums=dec_blk_nums)

        # self.to_img = nn.Sequential(
        #     nn.Conv2d(out_dim, dim, 3, 1, 1),
        #     nn.PixelShuffle(upscaling_factor)
        # )
        self.merge2 =  nn.Sequential(  # 这里考虑加一个MOE
            nn.Conv2d(mid_dim, out_dim-dim, 3, 1, 1),
        )
        self.merge1 =  nn.Sequential(  # 这里考虑加一个MOE
...
class FFT_filter(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # 2. 生成滤波器并应用于FFT频谱
        x2_fft = fft.fft2(x2)
        x2_fft_shifted = fft.fftshift(x2_fft)  # 将频谱中心移到图像中心
        
        outputs2 = []
        for i in range(4):
            filters = self.filter_generators[i](x1_splits[i])
            
            filtered_fft = x2_fft_shifted * filters
            output_ifft =  torch.abs(fft.ifft2(fft.ifftshift(filtered_fft)))
            output_ifft = output_ifft + x2
            outputs2.append(output_ifft)  # 取实部
        ...
        output1 = x1 * channel_weight  # 调制输入2
...
if __name__== '__main__': 
    ...
    model = dwt_kl8_7_3_3m_naf(dim=24,  upscaling_factor=8,dwt_dim=3,num_heads=3,mid_dim=36,
                         vae_weight='/data/liuyidi/nitre_2023_dehaze/data_dehaze/uhdformer/experiments/resfour9|moire/models/net_g_latest.pth',
                         config='/data/liuyidi/nitre_2023_dehaze/data_dehaze/uhdformer/experiments/resfour9|moire/8_16_res.yml').to('cuda')
