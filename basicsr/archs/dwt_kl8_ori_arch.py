...
class AutoencoderKL(nn.Module):
    def __init__(self,
        ...
        self.adapter_mid = nn.Sequential(make_attn(64, attn_type="restormer"),
                                         nn.Conv2d(64, 16, 1),
                                     make_attn(16, attn_type="restormer"))
    ...
    def encode(self, x):
        h,add ,high_list = self.encoder(x)
        add = self.adapter_mid(add)
        moments = self.quant_conv(h)
        
        return moments,add,high_list

    def decode(self, z,add,high_list):
        z = self.post_quant_conv(z)
        dec = self.decoder(z,add,high_list)
        return dec
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            self.adapters.insert(0, dwt_revadapter(16,16, block_out))

        # end
        self.norm_out = Normalize(block_in)
        self.norm_out2 = Normalize(16)
        ...
        self.conv_out2 = torch.nn.Conv2d(16,
                                        out_ch,
                                        kernel_size=3,
                                        stride=1,
                                        padding=1)

    def forward(self, z,add,high_list):
        ...
        if self.give_pre_end:
            return h
        det_out = self.conv_out2(nonlinearity(self.norm_out(add_in)))
        h = self.norm_out(h)
        h = nonlinearity(h)
        h = self.conv_out(h)
        if self.tanh_out:
    ...
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        for i_level in range(self.num_resolutions):
            ...
            if i_level != self.num_resolutions - 1:
                ...
                self.adapters.append(dwt_adapter(16,16,block_out))
...
if __name__== '__main__': 
    ...
    model = dwt_kl8(dim=48, n_blocks=8, ffn_scale=2.0, upscaling_factor=8,
