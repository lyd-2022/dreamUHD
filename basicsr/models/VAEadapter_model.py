...
class VAEadapter(BaseModel):
    def __init__(self, opt):
        ...
        # define network
        self.net_g = build_model(opt['network_g'])
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('encoder_opt'):
            train_opt['encoder_opt']['vae_weight'] = self.opt['network_g']['vae_weight']
            train_opt['encoder_opt']['vae_config'] = self.opt['network_g']['config']
            
            
            self.cri_encoder = build_loss(train_opt['encoder_opt']).to(self.device)
            a = 0
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output ,self.posterior = self.net_g(self.lq)
        ...
        if train_opt.get('encoder_opt', None):
            l_encoder = self.cri_encoder(self.posterior, self.gt)
            # l_encoder = self.cri_encoder(self.posterior, self.lq)
            l_g_total += l_encoder
            loss_dict['l_encoder'] = l_encoder
