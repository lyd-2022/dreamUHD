...
class FeMaSRModel(BaseModel):
    ...
    def test(self,current_iter,tb_logger):
        self.net_g.eval()
        net_g = self.get_bare_model(self.net_g)
        min_size = 2000 * 2000  # use smaller min_size with limited GPU memory
        lq_input = self.lq
        # restoration = self.net_g(self.lq)
        _, _, h, w = lq_input.shape
        if current_iter == -1 and tb_logger is None:
            with torch.no_grad():
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            self.test(current_iter,tb_logger)

            sr_img = tensor2img(self.output)
            metric_data = [img2tensor(sr_img).unsqueeze(0) / 255, self.gt]
            # if save_img and current_iter==-1 and tb_logger is None:
            #     print(metric_data )
            ...
            if save_img and current_iter % self.opt['val']['save_img_freq'] == 0:
            ...
            if save_img and current_iter==-1 and tb_logger is None:
                save_img_path = osp.join(
                    self.opt['path']['visualization'],'results',
                    f'{img_name}.png')
                # print(save_img_path)
                imwrite(sr_img, save_img_path)
