...
def load_resume_state(opt):
    ...
    if opt['auto_resume']:
        state_path = osp.join('/data/liuyidi/nitre_2023_dehaze/data_dehaze/uhdformer/experiments', opt['name'], 'training_states')
...
def train_pipeline(root_path):
    ...
    if resume_state:  # resume training
        ...
        # 
        # ume_state的上两级目录
        opt['path']['pretrain_network_g'] = osp.dirname(osp.dirname(opt['path']['resume_state']))+f'/model/net_g_{current_iter}.pth'
        #载入权重
        model.load_network(model.net_g, opt['path']['pretrain_network_g'], resume=True)
    else:
        start_epoch = 0
        current_iter = 0
        # model.resume_training(resume_state)  # handle optimizers and schedulers
