...
def main():
    ...
    parser.add_argument('-w', '--weight', type=str,
                        default='/fs-computility/ai4sData/liuyidi/code/DreamUHD/weight/DreamUHD_lowlight.pth',
                        help='path for model weights')
    ...
    parser.add_argument('-c', '--config', type=str,default='/fs-computility/ai4sData/liuyidi/code/DreamUHD/options/DreamUHD_LL.yml', help='path to the config file')
    ...
    if config['train'].get('ema_decay', None):
        ...
        EnhanceNet.load_state_dict(torch.load(enhance_weight_path)['params'], strict=True)
        # EnhanceNet.load_state_dict(torch.load(enhance_weight_path)[53], strict=True)
        # weight = torch.load(enhance_weight_path)

        # new_state_dict = {}

        # torch.save(weight, enhance_weight_path)
        # if 53 in weight:
        #     new_state_dict['params'] = weight[53]
        # torch.save(new_state_dict, '/fs-computility/ai4sData/liuyidi/code/DreamUHD/weight/DreamUHD_lowlight2.pth')
        # msg=EnhanceNet.load_state_dict(torch.load(enhance_weight_path), strict=False)
        # print('unmatched keys:', msg.unexpected_keys)
    ...
    for idx, path in enumerate(paths):
        ...
        if args.test_tile:
            with torch.no_grad():
                import time
                t0 = time.time()
                output = test_tile(EnhanceNet, img_tensor,1024)
                t1 = time.time()
                print('time:', t1-t0)
        else:
            with torch.no_grad():
